...
class RequestWrapper:
    ...
    @staticmethod
    @RequestErrorHandler.handle_http_errors
    def get_channels(key: str, parts: List[str], username: str = None, channel_ids: List[str] = None,
                     max_results: int = 5, page_token: str = None, localization_code: str = None) -> requests.Response:
        ...
        :param channel_ids: List of the YouTube channel ID(s) for the resource(s) that are being retrieved.
        ...
        if username:
            param_dict["forUsername"] = username
        elif channel_ids:
            param_dict["id"] = ",".join(channel_ids)
        else:
            raise ValueError("Missing required parameter (username or channel_ids)")
        if page_token:
    ...
    @staticmethod
    @RequestErrorHandler.handle_http_errors
    def get_comment_responses(key: str, parts: List[str], parent_id: str, max_results: int = 20, page_token: str = None,
                              text_format: str = "html") -> requests.Response:
        ...
        :param key: Required API key.
        :param parts: Comment resource properties that the API response will include.
        :param parent_id: ID of the comment for which replies should be retrieved.
